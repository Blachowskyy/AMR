...
class WozekModbusDriver:
    def __init__(self):
        ...
        try:
            #Biblioteka pyModbusTCP ktora nie przyjmuje wartosci ujemnych
            # self.deltaPLC = client.ModbusClient(host='192.168.1.4', port=502)
            # self.deltaPLC.open()

            #Biblioteka easymodbus przyjmuje wartosci ujemne, wyrzucala blad BrokenPipe 32
            self.deltaPLC = modbusClient.ModbusClient('192.168.1.5', 502)
            self.deltaPLC.connect()
            

            # self.ecoder = modbusClient.ModbusClient('192.168.1.5', 502) #zostal wypiety 
            # self.ecoder.connect()
            rospy.loginfo("Modbus setup complete")
            #subscribers/publishers 
            # self.pub_comand_curtis_vel = rospy.Publisher('wozek_angle_tick',Int64, queue_size=10, latch=True) #nie publikujemy odczytow 
    ...
    def validation_connection(self):
        try:
            while self.deltaPLC.is_connected() == False:
                rospy.loginfo('while')
                #self.deltaPLC.close()
                sleep(0.1)
            ...
            rospy.loginfo('error connect')
            

    #FIZYCZNY ZAKRES WOZKA 70 STOPNI
    def MoveServo(self, angle_impulses, action_type):  
        # self.validation_connection()
        # self.deltaPLC.connect()
        # rospy.loginfo('MoveServo')
        ...
        try:
            ...
            elif self.angle_impulses == self.Servo_Zeroj:
                self.validation_connection()  #Servo_Zeroj = 3
